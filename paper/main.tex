...
\begin{itemize}
  ...
  \item \textsc{boolq}: $0.565 \pm 0.023$ throughout; we verified
    that this is a \emph{data-and-capacity} limit, not a compute
    limit — a 2000-step run on 500 samples (seed 0) drives the
    training loss to $4 {\times} 10^{-6}$ (full memorization)
    while eval accuracy only moves to $0.592$, i.e.\ the LoRA
    memorizes the 500 training samples without generalizing.
    Richer adapters (higher rank) or more training data would
    be needed for \textsc{boolq} to escape chance.
\end{itemize}
