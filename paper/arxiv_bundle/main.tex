...
Wasserstein-regularized path); (ii) the flow induces emergent
species-level specialization when the attraction-to-species-regions is
strong enough, with a measured entropy gap of $1.40$ bits below the
maximum at a tuned hyperparameter point, versus $\sim\!0$ bits under the
default flat potentials; and (iii) a compact neural surrogate distilled
from the flow meets a sub-10\,ms p50 streaming-inference budget on
Apple Silicon with a measured p50 of $0.04$\,ms. We additionally report
a measured sweep of the entropic-Sinkhorn bias as a function of
$\varepsilon$. The companion repository contains the full implementation,
...
We next swept the potential magnitude $\alpha$, gradient-descent strength
$\beta$, and Turing coupling $\gamma$ over 27 configurations
($\alpha \in \{0, 1, 5\}$, $\beta \in \{0, 0.1, 1\}$,
$\gamma \in \{0, 1, 5\}$) on a reduced problem (grid 16, 30 slow steps)
with \emph{asymmetric} species potentials so that each species prefers a
distinct spatial region. At $(\alpha, \beta, \gamma) = (5, 1, 0)$ the mean
final entropy drops to $2.60$ bits against a maximum of $4$ bits
(gap $1.40$ bits, or $35\%$ below maximum), demonstrating that the
formalism \emph{does} support emergent specialization once the
attraction-to-species-preferred-regions is strong enough. The Turing
coupling $\gamma$ has negligible effect at this scale
(Figure \ref{fig:turing}). Full sweep data is in
\texttt{paper/hyperparam\_sweep.json}.
